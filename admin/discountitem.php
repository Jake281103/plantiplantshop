...
// To check enter discount product if it exists or not in product table
function checkinproducttable($pid)
{
    try {
        $conn = connect();
...
function checkindiscounttable($pid)
{
    try {
        $conn = connect();
...
if ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['discountinsert'])) {
    $pid = $_POST['pid'];
    $percent = $_POST['percent'];
    $checkinproducttable = checkinproducttable($pid);
    $checkindiscoundtable = checkindiscounttable($pid);
    if ($checkinproducttable != null) {
        if ($checkindiscoundtable) {
            $actualprice = $checkinproducttable['price'];
            $saveamount = $actualprice * ($percent / 100);
            $discountprice = $actualprice - $saveamount;
            try {
                $conn = connect();

                // To update the discount column in product table
                $sql = "UPDATE product SET discount=1 WHERE pid=$pid";
                $stmt = $conn->prepare($sql);
                $stmt->execute();

                $sql = "INSERT INTO discount (productid,discountpercent,discountprice) VALUES (?,?,?)";
                $stmt = $conn->prepare($sql);
                $stmt->execute([$pid, $percent, $discountprice]);
                $_SESSION['discount_insert_success'] = "New Discount Product is successfully inserted";
                $conn = null;
            } catch (PDOException $e) {
                echo $e->getMessage();
            }
        } else {
            $_SESSION['discount_product_exist'] = "Discount Product is already existed";
        }
    } else {
...
if ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['discountupdate'])) {
    $dpid = $_POST['dpid'];
    $prepid = $_POST['prepid'];
    $pid = $_POST['pid'];
    $percent = $_POST['percent'];
        ...
        if ($prepid != $pid) {
            ...
            if ($checkinproducttable != null) {
                ...
                if ($checkindiscoundtable) {
                    ...
                    // update discoult column of previous product
                    $sql = "UPDATE product SET discount=0 WHERE pid=$prepid";
                    $stmt = $conn->prepare($sql);
                    $stmt->execute();

                    // update discoult column of new discount product
                    $sql = "UPDATE product SET discount=1 WHERE pid=$pid";
                    $stmt = $conn->prepare($sql);
                    $stmt->execute();

                    // update in discount table
                    $sql = "UPDATE discount SET productid=$pid, discountpercent=$percent, discountprice=$discountprice WHERE dpid=$dpid";
                    $stmt = $conn->prepare($sql);
                    $stmt->execute();

                    $_SESSION['discount_update_success'] = "Discount Product is successfully Updated";
            ...
            $checkinproducttable = checkinproducttable($prepid);
            $actualprice = $checkinproducttable['price'];
            $saveamount = $actualprice * ($percent / 100);
            $discountprice = $actualprice - $saveamount;
...
if ($_SERVER['REQUEST_METHOD'] == "GET" && isset($_GET['deleteid']) && isset($_GET['pid'])) {
    ...
    try {
        ...
        // update discount column from product table
        $sql = "UPDATE product SET discount=0 WHERE pid=$pid";
        $stmt = $conn->prepare($sql);
        $stmt->execute();

        // delete discount product from discount table
        $sql = "DELETE FROM discount WHERE dpid=$dpid";
        $stmt = $conn->prepare($sql);
        $stmt->execute();

        $_SESSION['discount_delete_success'] = "Discount ID(" . $dpid . ") has been successfully deleted";
...
$conn = connect();
$sql = "SELECT * FROM discount LIMIT $offset, $productsPerPage";
$stmt = $conn->query($sql);

// Calculate the total number of products with the search term
$totalProductsQuery = "SELECT COUNT(*) AS total FROM discount";
...
<body>
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-2 col-md-3 fixed-top vh-100 overflow-auto leftsidebars">
                            <ul class="navbar-nav flex-column mt-4">
                                ...
                                <li class="nav-item">
                                    <a href="./discountitem.php" class="nav-link text-white p-3 mb-2 sidebarlinks currents"><i class="fas fa-tags me-3"></i> Add Discount Item</a>
                                </li>
                ...
                <div class="col-lg-10 col-md-9 ms-auto">
                    ...
                    <div class="row pt-md-5 mt-md-3">
                        ...
                        <div class="col-md-6 mb-2">
                            <div class="card shadow">
                                <div class="card-body px-4">
                                    <h4 class="card-title text-success mb-4">Insert Discount Product</h4>
                                    <form action="./discountitem.php" method="post">
                                        ...
                                        <div class="form-group px-3 mb-4">
                                            <label for="percent" class="h6 pb-1">Discount Percent</label>
                                            <input type="number" name="percent" id="percent" class="form-control rounded-0 border-success" min="1" max="100" placeholder="Enter Discount Percent" required>
                                        </div>
                                        <div class="px-3">
                                            <button type="submit" name="discountinsert" class="btn btn-success rounded-0 me-3">Insert</button>
                        ...
                        <div class="col-md-6 mb-2">
                            <div class="card shadow">
                                <div class="card-body px-4">
                                    <h4 class="card-title text-success mb-4">Update Discount Product
                                        <?php if (isset($_GET['updateid']) && isset($_GET['pid']) && isset($_GET['percent'])) { ?>
                                            (ID - <?php echo $_GET['updateid'] ?> )
                                        <?php } ?>
                                    </h4>
                                    <form action="./discountitem.php" method="post">
                                        <?php if (isset($_GET['updateid']) && isset($_GET['pid']) && isset($_GET['percent'])) { ?>
                                            <input type="hidden" name="dpid" value="<?php echo $_GET['updateid'] ?>" />
                                        <?php } ?>
                                        <?php if (isset($_GET['updateid']) && isset($_GET['pid']) && isset($_GET['percent'])) { ?>
                                        ...
                                        <div class="form-group px-3 mb-4">
                                            ...
                                            <?php } else { ?>
                                                <!-- <input type="text" name="pid" id="updatepid" class="form-control rounded-0 border-success" placeholder="Enter Product ID" required> -->
                                                <select class="form-select form-select-md border-success rounded-0 mb-3" name="pid" id="updatepid" required>
                                                    <option selected disabled>Select Product Id</option>
                                                    <?php foreach ($products as $product) { ?>
                                                        <option value="<?php echo $product['pid'] ?>"><?php echo ucfirst($product['pid']) ?> - <?php echo ucfirst($product['pname']) ?></option>
                                        ...
                                        <div class="form-group px-3 mb-4">
                                            <label for="updatepercent" class="h6 pb-1">Discount Percent</label>
                                            <?php if (isset($_GET['updateid']) && isset($_GET['pid']) && isset($_GET['percent'])) { ?>
                                                <input type="number" name="percent" id="updatepercent" class="form-control rounded-0 border-success" min="1" max="100" value="<?php echo $_GET['percent'] ?>" required>
                                            <?php } else { ?>
                                                <input type="number" name="percent" id="updatepercent" class="form-control rounded-0 border-success" min="1" max="100" placeholder="Enter Discount Percent" required>
                                            <?php } ?>
                                        </div>
                                        <div class="px-3">
                                            <?php if (isset($_GET['updateid']) && isset($_GET['pid']) && isset($_GET['percent'])) { ?>
                                                <button type="submit" name="discountupdate" class="btn btn-success rounded-0 me-3">Update</button>
                                            <?php } else { ?>
                                                <button type="submit" name="discountupdate" class="btn btn-success rounded-0 me-3" disabled>Update</button>
                                            <?php } ?>
                                            <button type="reset" class="btn btn-danger rounded-0">Reset</button>
                                        </div>
                                    </form>
                                        ...
                                        <tbody>
                                            ...
                                            ?>
                                                    <tr class="text-center align-middle">
                                                        <th scope="row"><?php echo $row['dpid'] ?></th>
                                                        <td><?php echo $row['productid'] ?></td>
                                                        <td><?php echo $row['discountpercent'] ?>%</td>
                                                        <td>$<?php echo $row['discountprice'] ?></td>
                                                        <td>
                                                            <a href="discountitem.php?updateid=<?php echo $row['dpid'] ?>&pid=<?php echo $row['productid'] ?>&percent=<?php echo $row['discountpercent'] ?>" class="btn btn-warning btn-sm">Update</a>
                                                            <a href="discountitem.php?deleteid=<?php echo $row['dpid'] ?>&pid=<?php echo $row['productid'] ?>" class="btn btn-danger btn-sm">Delete</a>
                                                        </td>
                                                    </tr>
                                            <?php }
                                            } ?>
    ...
    $alertmessage = null;
    $gifpath = null;
    if (isset($_SESSION['discount_insert_success'])) {
        $alertmessage = $_SESSION['discount_insert_success'];
    ...
    } elseif (isset($_SESSION['discount_product_exist'])) {
        $alertmessage = $_SESSION['discount_product_exist'];
        $gifpath = "./../../assets/img/gif/fail1.gif";
    } elseif (isset($_SESSION['discount_update_success'])) {
        $alertmessage = $_SESSION['discount_update_success'];
        $gifpath = "./../../assets/img/gif/success.gif";
    } elseif (isset($_SESSION['discount_delete_success'])) {
        $alertmessage = $_SESSION['discount_delete_success'];
        $gifpath = "./../../assets/img/gif/success.gif";
    }
    ...
    <?php
    if (
        isset($_SESSION['discount_insert_success']) || isset($_SESSION['product_not_found']) || isset($_SESSION['discount_product_exist'])
        || isset($_SESSION['discount_update_success']) || isset($_SESSION['discount_delete_success'])
    ) {
    ?>
    ...
    <?php
        unset($_SESSION['discount_insert_success']);
        unset($_SESSION['product_not_found']);
        unset($_SESSION['discount_product_exist']);
        unset($_SESSION['discount_update_success']);
        unset($_SESSION['discount_delete_success']);
    } ?>

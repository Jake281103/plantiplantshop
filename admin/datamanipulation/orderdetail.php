...
if ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['search'])) {
    ...
    try {
        ...
        $searchsql = "WHERE orderid=? ORDER BY orderid";

        $searchtotal = "FROM orderlines WHERE orderid=?";
    } catch (PDOException $e) {
        echo $e->getMessage();
    }
}else{
    $searchsql = "ORDER BY orderid";
    $searchtotal = "FROM orderlines";
}
$sql = "SELECT * FROM orderlines {$searchsql} LIMIT $offset, $productsPerPage";
$stmt = $conn->prepare($sql);
...
$totalProductsQuery = "SELECT COUNT(*) AS total {$searchtotal}";
$totalProductsStmt = $conn->prepare($totalProductsQuery);
if (isset($_POST['search'])) {
    $totalProductsStmt->execute([$orderid]);
}else{
                                    ...
                                    <div>
                                        <form action="./orderdetail.php" method="post">
                                            <div class="d-flex justify-content-center align-items-center">
                                                <input type="number" name="orderid" class="form-control form-control-sm border-success rounded-0 me-1" min="1" placeholder="Enter Order Id" required />
                                                <button type="submit" name="search" class="btn btn-success btn-sm rounded-0">Search</button>
                                            </div>
                                        </form>
                                    </div>
                                    <div class="mt-1">
                                        ...
                                        <tbody>
                                            ...
                                            ?>
                                                    <tr class="text-center align-middle">
                                                        <th scope="row"><?php echo $row['olid'] ?></th>
                                                        ...
                                                        try {
                                                            ...
                                                            $sql = "SELECT pname FROM product WHERE pid = $pid";
                                                            $stmt = $conn->prepare($sql);
                                                            $stmt->execute();
                                                            $pnames = $stmt->fetch();
                                                            $pname = $pnames['pname'];

                                                            $sql = "SELECT url FROM productimage WHERE productid = $pid";
                                                            $stmt = $conn->prepare($sql);
                                                            $stmt->execute();
                                                        ...
                                                        <td>
                                                            <img src="./../../<?php echo $imageurl ?>" width="45" alt="<?php echo $pid ?>" />
                                                        </td>
                                                        <td><?php echo $row['quantity'] ?></td>
                                                    </tr>
                                            <?php }
                                            }else{ ?>
                                                <tr class="align-center text-center py-5" >
                                                    <td class="py-5" colspan="6">Order Is not Found</td>
                                                </tr>

...
if ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['search'])) {
    ...
    try {
        ...
        $searchsql = "WHERE odid=? ORDER BY date";

        $searchtotal = "FROM orders WHERE odid=?";
    } catch (PDOException $e) {
        echo $e->getMessage();
    }
}else{
    $searchsql = "ORDER BY date";
    $searchtotal = "FROM orders";
}
$sql = "SELECT * FROM orders {$searchsql} LIMIT $offset, $productsPerPage";
$stmt = $conn->prepare($sql);
                                        ...
                                        <tbody>
                                            ...
                                            ?>
                                                ...
                                                <tr class="align-center text-center py-5" >
                                                    <td class="py-5" colspan="9">Order Is not Found</td>
                                                </tr>
